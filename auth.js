/* ==========================================================================
   EcoTrace — Auth & Business Onboarding
   Simulated phone + 4-digit SMS OTP login, followed by a one-time business
   profile capture (name, state, city, PIN code). Runs after script.js, so
   it can read/patch the existing app (state, renderSettings, …) without
   editing it. Everything here is gated behind the pre-paint check in
   index.html's <head> script, which adds html.ecotrace-authed for
   returning, already-onboarded users so there is no flash of the app.
   ========================================================================== */
(function () {
  var ACCOUNT_KEY = 'ecotrace.account';
  var pendingPhone = '';
  var pendingCode = '';
  ...
  /* Push the onboarded profile into the existing app's own storage/state,
     the same keys script.js already reads — no edits to script.js needed. */
  function syncAccountIntoApp(acc) {
    try {
      var raw = localStorage.getItem('ecotrace.settings');
      var settings = raw ? JSON.parse(raw) : {};
      settings.businessName = acc.businessName;
      localStorage.setItem('ecotrace.settings', JSON.stringify(settings));
    } catch (e) { /* ignore */ }
    try {
      localStorage.setItem('ecotrace.businessLocation', JSON.stringify({ state: acc.state, city: acc.city, pin: acc.pin }));
    } catch (e) { /* ignore */ }

    if (typeof state !== 'undefined' && state && state.settings && state.settings.businessName !== acc.businessName) {
      state.settings.businessName = acc.businessName;
  ...
  function showStep(root, step) {
    var ids = { phone: 'authStepPhone', otp: 'authStepOtp', onboard: 'authStepOnboard' };
  ...
  function wireAuthEvents(root) {
    ...
    onboardForm.addEventListener('submit', function (e) {
      ...
      if (!businessName || !st || !city) return;
      var acc = { phone: pendingPhone, businessName: businessName, state: st, city: city, pin: pin, createdAt: new Date().toISOString() };
      saveAccount(acc);
      syncAccountIntoApp(acc);
      finishAuth();
    });
  }
  ...
  /* ---- Settings page: append a business-account card without touching
     the existing renderSettings() body. ---- */
  function injectAccountCard() {
    var grid = document.querySelector('#page-settings .settings-grid');
    var acc = getAccount();
    if (!grid || !acc || grid.querySelector('#accountCard')) return;
    var card = document.createElement('section');
    card.className = 'card card--pad';
    card.id = 'accountCard';
    card.innerHTML =
      '<div class="card__head"><div><h2>' + esc(t('settings.account_title')) + '</h2><p>' + esc(t('settings.account_desc')) + '</p></div></div>' +
      '<div class="setting-row"><div><strong>' + esc(t('settings.account_phone')) + '</strong><p>+91 ' + esc(acc.phone) + '</p></div></div>' +
      '<div class="setting-row"><div><strong>' + esc(t('settings.account_location')) + '</strong><p>' + esc([acc.city, acc.state, acc.pin].filter(Boolean).join(', ')) + '</p></div>' +
